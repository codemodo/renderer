...
{
    glm::vec3 p0 = glm::vec3(transform.T() * glm::vec4(points[0], 1));
    glm::vec3 p1 = glm::vec3(transform.T() * glm::vec4(points[1], 1));
    glm::vec3 p2 = glm::vec3(transform.T() * glm::vec4(points[2], 1));
    area = Area(p0, p1, p2);
}

Intersection Triangle::GetSampledIntersection(float randX, float randY, const glm::vec3 &isx_normal)
{
    Intersection isc;
    return isc;
}
...
Intersection Mesh::GetSampledIntersection(float randX, float randY, const glm::vec3 &isx_normal)
{
    Intersection isc;
    return isc;
}
...
Intersection Mesh::GetIntersection(Ray r){
    ...
    if(closest.object_hit != NULL)
    {
        ...
        glm::vec3 T = glm::normalize(glm::cross(glm::vec3(0,1,0), glm::vec3(tri->GetNormal(P))));
        glm::vec3 B = glm::cross(glm::vec3(tri->GetNormal(P)), T);
        closest.tangent = glm::normalize(glm::vec3(transform.T() * glm::vec4(T, 0)));
        closest.bitangent = glm::normalize(glm::vec3(transform.T() * glm::vec4(B, 0)));
    }
    return closest;
}
