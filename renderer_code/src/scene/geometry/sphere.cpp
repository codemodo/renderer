...
{
    glm::vec4 p1(-0.5), p2(0.5);
    BoundingBox *bb;
    bb = new BoundingBox(glm::vec3(transform.T() * p1), glm::vec3(transform.T() * p2));
    return *bb;
}
...
float Sphere::RayPDF(const Intersection &isx, const Ray &ray) {
    ...
    float cosThetaMax = glm::sqrt(std::max(0.f, 1.f - sinThetaMax2));
...
Intersection Sphere::GetSampledIntersection(float u1, float u2, const glm::vec3 &isx_normal)
{
    glm::vec3 isx_normal_tmp;
    isx_normal_tmp = glm::vec3(this->transform.invT() * glm::vec4(isx_normal,0));
    float z = 1.f - 2.f * u1;
    ...
    if(glm::dot(isx_normal_tmp, normal3) > 0)
    {
        normal3 = -normal3;
    }
...
Intersection Sphere::GetIntersection(Ray r)
{
    ...
    if(t >= 0)
    {
        ...
        glm::vec3 normal = glm::normalize(glm::vec3(P - glm::vec4(0,0,0,1)));
        ...
        glm::vec3 T = glm::normalize(glm::cross(glm::vec3(0,1,0), glm::vec3(normal)));
        glm::vec3 B = glm::cross(glm::vec3(normal), T);
        result.tangent = glm::normalize(glm::vec3(transform.T() * glm::vec4(T, 0)));
        result.bitangent = glm::normalize(glm::vec3(transform.T() * glm::vec4(B, 0)));
