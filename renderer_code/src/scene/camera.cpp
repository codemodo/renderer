...
Ray Camera::RaycastNDC_DOF(float ndc_x, float ndc_y)
{
    // keep track of origin and point of focus across samples
    glm::vec3 Origin, POF;

    // focal factor representing focal distance (1.0 corresponds to the origin)
    float focalFactor = 0.3f;
    ...
    for (int i = 0; i < samples; i++)
    {
        ...
        glm::vec3 USD = la::UnifSampleDisc(rand1, rand2);
        float xUSD = USD.x;
        float yUSD = USD.y;
        ...
        glm::vec3 tmpOrigin, tmpDirection;
        tmpOrigin = eye; tmpDirection = P - eye;
        tmpOrigin.x += xUSD;  tmpOrigin.y += yUSD;

        glm::vec3 tmpPOF = tmpDirection * focalFactor + eye;

        Origin+=tmpOrigin;  POF += tmpPOF;
    }

    Origin /= samples; POF /= samples;
    Ray result(Origin, POF - Origin);
    return result;
}
