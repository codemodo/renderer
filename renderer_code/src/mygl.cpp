...
#else
    StratifiedPixelSampler pixel_sampler(scene.sqrt_samples,time(0));

    for(unsigned int i = 300; i < 305; i++)
    {
        for(unsigned int j = 250; j < 271; j++)
        {
            //std::cout << i << " " << j << "\n";
...
//        {
    //int i = 500; int j = 200;
            ...
            {
                glm::vec3 color = integrator.TraceRay(scene.camera.Raycast(sample_points[a]), 5);
                accum_color += color;
            }
